...
/**
 * Regression guard for crash candidate #21: crash-buildongoingcallnotification-context-null.
 *
 * Before the fix, PushwooshCallUtils.buildOngoingCallNotification did a Kotlin non-null cast
 ...
 * a null context now makes buildOngoingCallNotification return null, and onStartCommand skips the
 * foreground start and returns START_NOT_STICKY. These tests assert that graceful behavior.
 */
@RunWith(RobolectricTestRunner::class)
class BuildOngoingCallNotificationNullContextTest {
    ...
    fun directCall_nullContext_returnsNullGracefully() {
        Mockito.mockStatic(AndroidPlatformModule::class.java).use { mocked ->
            ...
            val notification = PushwooshCallUtils.buildOngoingCallNotification(null)
    ...
    // context, onStartCommand for the ONGOING action does not throw, returns START_NOT_STICKY, and
    ...
    // (2) with a non-null context, onStartCommand for the ONGOING action completes without throwing
    //     and returns START_NOT_STICKY (the foreground machinery may no-op under Robolectric) → the
    //     happy reach path still works and the guard did not divert it into the stopSelf() branch.
    @Test
    fun negativeControl_service_nonNullContext_completesNormally() {
        val realContext: Context = RuntimeEnvironment.getApplication()
        Mockito.mockStatic(AndroidPlatformModule::class.java).use { mocked ->
            mocked.`when`<Context?> { AndroidPlatformModule.getApplicationContext() }.thenReturn(realContext)
            val service = Robolectric.buildService(PushwooshCallService::class.java).create().get()
            val intent = Intent().setAction(Constants.PW_POST_ONGOING_CALL_ACTION)
            val result = service.onStartCommand(intent, 0, 1)
            assertEquals("non-null context must complete the happy path and return START_NOT_STICKY", Service.START_NOT_STICKY, result)
        }
    }
}
