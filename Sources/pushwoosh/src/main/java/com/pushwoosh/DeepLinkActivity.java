...
public class DeepLinkActivity extends TranslucentActivity {
    ...
    private void createTestDevice(final Context context) {
        ...
        CreateTestDeviceRequest request =
                new CreateTestDeviceRequest(DeviceUtils.getDeviceName(), "Imported from the app", false);
        NetworkModule.getRequestManager().sendRequest(request, result -> {
            if (result.isSuccess()) {
                Toast.makeText(context, "Test device has been registered.", Toast.LENGTH_LONG)
                        .show();
            } else {
                Toast.makeText(
                                context,
                                "Test device registration has failed. "
